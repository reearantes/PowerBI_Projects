...
### **Explicação da Formatação no Markdown**

- **Títulos**: Usei `#` para títulos principais, `##` para subtítulos e `###` para títulos menores.
- **Negrito**: Usei `**texto**` para destacar palavras ou frases.
- **Listas**: Usei `-` para listas não ordenadas.
- **Código**: Usei `` ` `` para destacar trechos de código ou nomes de campos.
